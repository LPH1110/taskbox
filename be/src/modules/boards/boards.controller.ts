...
import { createBoardSchema, boardIdParamSchema, updateBoardSchema, updateBoardGithubSchema } from "./boards.schema";
...
// PATCH /api/boards/:boardId/github (Link/Unlink GitHub repo)
router.patch("/:boardId/github", requireBoardMember, validate(updateBoardGithubSchema), async (req: Request, res: Response, next: NextFunction) => {
  const { boardId } = req.params;
  const { github_repo_full_name } = req.body;
  const userId = (req.user as any).id;
  ...
  try {
    ...
    if (!isAuthorized) {
      return sendError(res, "Forbidden: Only board owners or admins can modify GitHub linkage", 403);
    }
